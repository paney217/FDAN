...
def test_and_generate_result_round(model_name='resnet18_fdan', num_classes=5, img_size=224):
    ...
    state_dict = torch.load('./checkpoint/' + model_name  + '_N.ckpt', map_location=lambda storage, loc: storage.cuda())['state_dict']
    ...
    with open(os.path.join('checkpoint', model_name+'_N_'+RESULT_FILE), 'w', encoding='utf-8') as fd:

        fd.write('No. ' + 'image_name ' + 'gt'  + 'pred')

        ## Read test images
        test_file_csv = open(os.path.join(data_root, "Test_data_list_N.csv"))
